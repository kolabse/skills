...
CODEX_BLOCK = """<!-- maintain-work-log:start -->
## Work log
...
def inspect(project_path: Path, agent: str = "codex") -> dict[str, object]:
    ...
    agents = root / AGENTS[agent]["filename"]
...
def configure(project_path: Path, agent: str = "codex") -> tuple[dict[str, object], bool]:
    ...
    if not state["managed_block"]:
        ...
        agents.write_text(f"{text}{separator}{AGENTS[agent]['block']}\n", encoding="utf-8", newline="\n")
        changed = True
    log = root / LOG_RELATIVE
    if not log.is_file():
