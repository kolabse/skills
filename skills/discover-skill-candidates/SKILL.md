...
description: "Discover and rank reusable skill ideas from bounded project rules and user-approved evidence; export selected portable contribution packages or validate received skill-candidate JSON packages for maintainer review. Use when the user asks which rules, project patterns, Git history, documentation, current-chat practices, or approved summaries from supplied chat exports and sync-project-context handoffs should become skills; wants to compare ideas with existing skills, prepare an implementation brief, contribute an idea to kolabse/skills, or check an incoming candidate package. Do not use to create skills, validate unrelated JSON documents, enumerate chats implicitly, scan source code broadly, or turn secrets, organization policy, and one-off conventions into reusable workflows."
---
