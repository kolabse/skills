    ...
    def test_release_build_is_deterministic_and_contains_distribution(self) -> None:
        ...
        with tempfile.TemporaryDirectory() as first_directory:
            with tempfile.TemporaryDirectory() as second_directory:
                ...
                self.assertIn(prefix + ".agents/plugins/marketplace.json", zip_names)
                self.assertIn(prefix + ".agents/plugins/marketplace.json", tar_names)
                self.assertIn(prefix + ".claude-plugin/marketplace.json", zip_names)
                self.assertIn(prefix + ".claude-plugin/marketplace.json", tar_names)
                ...
                self.assertIn(prefix + "scripts/smoke_marketplaces.py", zip_names)
                ...
                self.assertIn(prefix + "scripts/bootstrap_update.py", zip_names)
                self.assertIn(prefix + "scripts/smoke_bootstrap.py", zip_names)
                self.assertIn(prefix + "schemas/manager-plan.schema.json", zip_names)
                self.assertIn(prefix + "schemas/manager-result.schema.json", zip_names)
                ...
                stable_baseline = "evals/baselines/release-holdout-v4-v1.17.0.json"
                self.assertIn(prefix + stable_baseline, zip_names)
                self.assertIn(prefix + stable_baseline, tar_names)
                self.assertIn(prefix + "evals/release-holdout-v4.json", zip_names)
                self.assertIn(prefix + "evals/release-holdout-v4.json", tar_names)
