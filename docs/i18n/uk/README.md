...
#### `maintain-work-plan` (експериментальний)

Підтримує постійний упорядкований план проєкту зі сталими ID, необов’язковими бажаними датами та залежностями.

- Під час першого запуску визначає наявні шляхи плану й журналу, код проєкту, часовий пояс і правила роботи з календарем; зберігає наявний Markdown.
- Працює разом із `maintain-work-log`: перевіряє завершення та зберігає датований запис у журналі перед вилученням виконаної роботи з активного плану.
- За наявності дозволу синхронізує завдання з датами з календарем, використовуючи заголовки `[КОД ПРОЄКТУ] Назва завдання`; виявляє конфлікти й дублікати та зберігає ручні зміни.
- Допоміжна програма лише читає нормалізовані дані, перевіряє їх і показує запропоновані зміни; вона не містить клієнта календаря та не доводить завершення завдань.

Установлюється стандартними командами встановлення колекції. Виклик:
